...
public final class InMemoryOrderMatcher implements OrderMatcher {
    ...
    public void listAsset(Listing a)
    {
        ...
        if(amountLeft.get() < 0) {
            ...
            orders.add(partialMatch.adjustQuantityTo(stillRequired));
        }
        
        if(amountLeft.get() > 0) {
            assets.add(a.adjustQuantityTo(amountLeft.get()));
        }
    }
    ...
    public void placeOrder(Listing o)
    {
        ...
        if(amountStillRequired.get() < 0) {
            ...
            assets.add(partialMatch.adjustQuantityTo(quantityLeft));
        }
        
        if(amountStillRequired.get() > 0) {
            orders.add(o.adjustQuantityTo(amountStillRequired.get()));
        }
    }
